...
import { logAuditAction } from '../../https';
...
// --- HELPER 2: PARSE IMPORT DATA ---
const parseImportRow = (row, adminUserId) => {
    // 1. CLEAN TOTAL
    const rawTotal = row['Total Amount'] || row['Total'] || 0;
    const totalAmount = cleanNumber(rawTotal);

    // 2. PARSE ITEMS
    const items = [];
    if (row['Items Summary'] && row['Items Summary'] !== 'No Items') {
        const parts = row['Items Summary'].split('),');
        parts.forEach(part => {
            const match = part.match(/(.+?)\s*\(x(\d+)\s*@\s*[^\d]*([\d.]+)/);
            if (match) {
                const name = match[1].trim();
                const quantity = parseInt(match[2]);
                const unitPrice = cleanNumber(match[3]);
                
                if (name && quantity && !isNaN(unitPrice)) {
                    items.push({ 
                        name: name, 
                        quantity: quantity, 
    ...
    if (items.length === 0 && totalAmount > 0) {
        items.push({ name: "Imported Item", quantity: 1, price: totalAmount });
    }
    ...
    return {
        // INJECT USER ID (Prevents Schema Rejection)
        user: adminUserId,       
        cashier: adminUserId,

        customerDetails: {
            name: row['Customer'] || 'Walk-in',
            contact: 'N/A'
        },
        items: items,
        bills: {
...
const ImportExportSection = () => {
    ...
    const handleDownloadTemplate = () => {
        ...
        worksheet['!cols'] = [{ wch: 20 }, { wch: 20 }, { wch: 50 }, { wch: 15 }, { wch: 15 }, { wch: 15 }, { wch: 15 }];
    ...
    const importMutation = useMutation({
        mutationFn: (ordersData) => api.post('/api/orders/bulk-import', { orders: ordersData }),
        onSuccess: async (res) => {
            const importedCount = res.data?.data?.length || 0;
            if (importedCount === 0) {
                enqueueSnackbar("Warning: 0 orders saved. (Database Rejected Data)", { variant: 'warning' });
            } else {
                enqueueSnackbar(`Success! ${importedCount} orders added to system.`, { variant: 'success' });
    ...
    const handleImportUpload = () => {
        ...
        // --- THE FIX: ROBUST USER ID FINDER ---
        let adminUserId = null;
        try {
            const userInfoStr = localStorage.getItem('userInfo');
            if (userInfoStr) {
                const userInfo = JSON.parse(userInfoStr);
                // Look everywhere for the ID
                adminUserId = userInfo._id || userInfo.id || (userInfo.user && (userInfo.user._id || userInfo.user.id));
            }
        } catch (e) { console.log(e); }

        // --- FALLBACK SYSTEM ID ---
        // If we still can't find it, use a valid 24-char Dummy ObjectID to bypass the error
        // 000000000000000000000000 is a valid Hex string for MongoDB
        if (!adminUserId) {
            console.warn("⚠️ Could not find Admin ID. Using System Fallback ID.");
        ...
        reader.onload = (evt) => {
            try {
                ...
                if (rawData.length === 0) {
                    enqueueSnackbar("Selected sheet is empty.", { variant: 'warning' });
                    return;
                }
                
                const cleanData = rawData.filter(r => r['Customer'] || r['Order ID'] || r['Total Amount']);
                
                // Pass the found (or fallback) ID to the parser
                const parsedOrders = cleanData.map(row => parseImportRow(row, adminUserId));
                
                console.log("SENDING CONVERTED DATA:", parsedOrders);
                importMutation.mutate(parsedOrders);

            } catch (err) {
                console.error("Parse Error:", err);
                enqueueSnackbar("Failed to parse file.", { variant: 'error' });
            }
        };
                    ...
                    <div className="space-y-6">
                        <div className="space-y-3">
                            <div className="flex justify-between items-end">
                                <div><h2 className="text-[#f5f5f5] text-lg font-bold">Required Excel Format</h2><p className="text-[#ababab] text-xs">Your file must strictly follow this column structure.</p></div>
                                ...
                                <table className="w-full text-left text-xs text-[#ababab]">
                                    ...
                                    <tbody>
                                        <tr className="border-b border-[#333] bg-[#1f1f1f]">
                                            ...
                                            <td className="px-4 py-2 text-[#ccc]">Burger (x2 @ ₱150)</td>
                        ...
                        <div className="border-2 border-dashed border-[#444] rounded-xl p-8 flex flex-col items-center justify-center hover:border-[#f6b100] transition-colors bg-[#1f1f1f]">
                            {!importFile ? (
                                <>
                                    ...
                                    <p className="text-[#555] text-xs mt-2">Supports .xlsx, .xls, .csv</p>
                                </>
                            ) : (
